...
createApp({
  setup() {
    ...
    const newPO = ref({ po_number: '', po_date: today(), customer_id: '', target_delivery: '', project: '', items: [{ part_number: '', part_name: '', qty_order: '', batches: [] }] });
    ...
    const pageTitle = computed(() => ({ dashboard: 'Dashboard', input: 'Input Stock', po: 'Input PO', porekap: 'Rekap PO', partlist: 'Daftar Part', history: 'History Transaksi', sjhistory: 'History Surat Jalan', auditlog: 'Audit Trail' })[page.value] || '');
    ...
    const editPartModal = ref({ show: false, loading: false, part: null, form: { part_number: '', part_name: '', model: '', commodity: '', supplier: '', min_stock: 1 } });
    function openEditPartModal(p) {
      editPartModal.value = {
        show: true, loading: false, part: p,
        form: {
          part_number: p.part_number, part_name: p.part_name, model: p.model || '',
          commodity: p.commodity, supplier: p.supplier || '', min_stock: p.min_stock,
        },
      };
    }
    async function submitEditPart() {
      const f = editPartModal.value.form;
      if (!f.part_number || !f.part_name || !f.commodity) { showToast('Part Number, Part Name, dan Commodity wajib diisi!', 'error'); return; }
      editPartModal.value.loading = true;
      try {
        await apiFetch('/parts/' + editPartModal.value.part.id, {
          method: 'PUT',
          body: JSON.stringify({
            part_number: f.part_number, part_name: f.part_name, model: f.model || null,
            commodity: f.commodity, supplier: f.supplier || null, min_stock: parseInt(f.min_stock) || 1,
          }),
        });
        showToast('Part berhasil diupdate!');
        editPartModal.value.show = false;
        await loadParts();
      } catch (err) { showToast(err.message, 'error'); }
      finally { editPartModal.value.loading = false; }
    }
    ...
    function addPOItemRow() { newPO.value.items.push({ part_number: '', part_name: '', qty_order: '', batches: [] }); }
    function removePOItemRow(idx) { newPO.value.items.splice(idx, 1); }

    function recalcItemQty(item) {
    ...
    async function handleImportPOItems(e) {
      ...
      try {
        ...
        let headerRowIdx = -1, pnCol = -1, batchCols = [];
        for (let r = 0; r < rows.length; r++) {
          const cells = rows[r].map(c => String(c).trim().toUpperCase());
          ...
          if (pi !== -1 && ni !== -1) {
            headerRowIdx = r;
            pnCol = pi;
            // Kumpulin semua kolom batch: "N", "N+1", "N+2", dst — berhenti begitu polanya putus
            let c = ni, batchNum = 1;
            while (c < cells.length && (cells[c] === 'N' || cells[c] === 'N+' + (batchNum - 1))) {
              batchCols.push(c);
              c++; batchNum++;
            }
            break;
        ...
        for (let r = headerRowIdx + 1; r < rows.length; r++) {
          ...
          batchCols.forEach(col => {
            const q = parseInt(row[col]);
            if (q && !isNaN(q) && q > 0) batches.push({ qty: q, target_date: '' });
          });
          ...
          imported.push({
            part_number: pn,
            part_name: match ? match.part_name : '',
            qty_order: totalQty,
    ...
    async function submitPO() {
      ...
      try {
        await apiFetch('/po', { method: 'POST', body: JSON.stringify({ po_number: newPO.value.po_number, po_date: newPO.value.po_date, customer_id: newPO.value.customer_id || null, target_delivery: newPO.value.target_delivery || null, project: newPO.value.project || null, items: items.map(it => ({
          part_number: it.part_number,
          qty_order: parseInt(it.qty_order),
          batches: (it.batches && it.batches.length > 1) ? it.batches.map(b => ({ qty: parseInt(b.qty), target_date: b.target_date || null })) : null,
        })) }) });
